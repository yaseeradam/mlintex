                ...
                data: (products) {
                  ...
                  return GridView.builder(
                    ...
                    itemBuilder: (_, i) {
                      ...
                      final inCart = cart.where((c) => c.product.id == product.id).firstOrNull;
                      return _ProductTile(
                        product: product,
            ...
            if (cart.isNotEmpty)
              _CartSummary(
                ...
                onCheckout: () async {
                  ...
                  final success = await ref.read(saleNotifierProvider.notifier).completeSale();
                  if (mounted) {
                    AppFeedback.hideLoading(context);
                    if (success) {
                      AppFeedback.showSuccess(context, 'Sale Completed!', 'The sale has been successfully recorded.');
                    } else {
                      AppFeedback.showError(context, 'Error', 'Failed to complete the sale.');
                    }
                  }
                },
              ),
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        child: GlassContainer(
          ...
          color: isInCart ? AppTheme.primaryColor.withOpacity(0.08) : null,
          border: isInCart
                        ...
                        Container(
                          ...
                          decoration: BoxDecoration(
                            gradient: LinearGradient(
                              colors: ProductStyleUtil.getStyle(product.category, product.name).colors,
                              begin: Alignment.topLeft,
                              end: Alignment.bottomRight,
                            ),
                            borderRadius: BorderRadius.circular(8),
                          ),
                          child: Icon(ProductStyleUtil.getStyle(product.category, product.name).icon, color: Colors.white, size: 14),
                        ),
